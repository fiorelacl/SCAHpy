...
```python
file_name = ['/data/users/fcastillon/COW_outs/diagnos/C2O5W4_SF2/wrf/wrfout_d01_2017-01-14_03:00:00']
```

**b) Using `glob.glob`, which already returns a list:**

```python
file_name = glob.glob('/data/users/fcastillon/COW_outs/diagnos/C2O5W4_SF2/wrf/wrfout_d01_2017-01-14_03:00:00')
```
...
In this example, we retain only land areas, so we use `sea_is_one = True` and `exclude_lakes = False`.
...
```python
WRF_files = sorted(glob.glob("/data/users/fcastillon/COW_outs/diagnos/C2O5W4_SF2/wrf/wrfout_d01_2017*"))
```
